...
import useMenProducts from '@/hooks/useMenProduct'
...
const ClothingMen = () => {
  const { data: products, isLoading, error } = useMenProducts();
  const [currentPage, setCurrentPage] = useState(1);
  if (isLoading) return <div>Loading...</div>;
  if (error) return <div>Error loading products</div>;
  if (!products || products.length === 0) return <div>No products available for Men</div>;
